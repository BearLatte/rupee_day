      ...
      if (controller.orders.value.isEmpty) {
        ...
        return SwipeRefresh.cupertino(
          ...
          children: controller.orders.value
              .map((order) => orderItem(order, addFeedbackAction: controller.addFeedbackAction))
              .toList(),
        );
      }
    });
      ...
      title: Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            Row(
              children: <Widget>[
                ...
                Column(
                  ...
                  children: <Widget>[
                    Text('₹ ${order.loanAmount}',
                        style: TextStyle(color: HexColor('#E7AF74'), fontSize: 22, fontWeight: FontWeight.w600)),
                    Text('Loan amount', style: TextStyle(color: HexColor('#999999'), fontSize: 16))
                  ],
                ),
                const Spacer(),
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('₹ ${order.loanApplyDate}',
                      style: TextStyle(color: HexColor('#15173B'), fontSize: 22, fontWeight: FontWeight.w600)),
            ...
            Text('Order Number : ${order.loanOrderNo}',
                textAlign: TextAlign.left, style: TextStyle(color: HexColor('#15173B'), fontSize: 16)),
            const Padding(padding: EdgeInsets.only(bottom: 10)),
            CommonTextButton('Feedback',
                width: 185,
                height: 40,
                borderRadius: 20,
                backgroundColor: HexColor('#15173B'),
                onTap: () => addFeedbackAction(order))
          ],
        ),
      ),
    );
